...
ALTER TABLE group_pricing_quotes ADD COLUMN IF NOT EXISTS continuation_option BOOLEAN;
-- Update column type if it exists
DO $$
BEGIN
    IF EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name='group_pricing_quotes' AND column_name='continuation_option') THEN
        ALTER TABLE group_pricing_quotes ALTER COLUMN continuation_option TYPE BOOLEAN;
    END IF;
END $$;
