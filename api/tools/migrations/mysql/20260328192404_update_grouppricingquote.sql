...
-- MySQL: Add or modify column
SET @s = (SELECT IF(
    EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='group_pricing_quotes' AND COLUMN_NAME='continuation_option' AND TABLE_SCHEMA = DATABASE()),
    'ALTER TABLE group_pricing_quotes MODIFY COLUMN continuation_option TINYINT(1);',
    'ALTER TABLE group_pricing_quotes ADD COLUMN continuation_option TINYINT(1);'
));
PREPARE stmt FROM @s;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
