...
func SaveTemplate(insurerID int, filename string, blob []byte, uploadedBy string) (*models.InsurerOnRiskLetterTemplate, error) {
	if len(blob) == 0 {
	...
	var maxVersion int
	err := tx.Model(&models.InsurerOnRiskLetterTemplate{}).
	...
	// Create new template
	template := &models.InsurerOnRiskLetterTemplate{
		InsurerID:  insurerID,
		Version:    newVersion,
		Filename:   filename,
		DocxBlob:   blob,
...
func GetActiveTemplate(insurerID int) (*models.InsurerOnRiskLetterTemplate, error) {
...
func ListTemplates(insurerID int) ([]models.InsurerOnRiskLetterTemplate, error) {
	var templates []models.InsurerOnRiskLetterTemplate
	err := services.DB.
...
func GetTemplate(id int) (*models.InsurerOnRiskLetterTemplate, error) {
	var template models.InsurerOnRiskLetterTemplate
...
func DeleteTemplate(insurerID, templateID int) error {
	...
	err := services.DB.Delete(&models.InsurerOnRiskLetterTemplate{}, templateID).Error
	if err != nil {
...
// never touched, so subsequent on-risk letter generation is unaffected.
func DeleteInactiveTemplates(insurerID int) (int64, error) {
	result := services.DB.
		Where("insurer_id = ? AND is_active = ?", insurerID, false).
		Delete(&models.InsurerOnRiskLetterTemplate{})
	if result.Error != nil {
