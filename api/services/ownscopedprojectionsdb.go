...
func (db *OwnDb) BatchInsertScopedProjections(objArr []models.ScopedAggregatedProjection) (int64, error) {
	// If there is no data, nothing to do.
	if len(objArr) == 0 {
		return 0, errors.New("slice must not be empty")
	}

	numberOfFields := db.calculateNumberOfFieldsScoped((objArr)[0])
	return db.batchInsertScopedChunks(splitIntoScopedChunks(objArr, numberOfFields))
}

func (db *OwnDb) batchInsertScopedChunks(chunks [][]models.ScopedAggregatedProjection) (int64, error) {
	var rowsAffected int64 = 0
	for _, chunk := range chunks {
		chunkRowsAffected, err := db.batchScopedInsert(chunk)
		if err != nil {
			return 0, err
		}
...
func splitIntoScopedChunks(objArr []models.ScopedAggregatedProjection, numberOfFields int) [][]models.ScopedAggregatedProjection {
	var chunks [][]models.ScopedAggregatedProjection
	...
	if numberOfObjects < chunkSize {
		return [][]models.ScopedAggregatedProjection{objArr}
	}
...
func (db *OwnDb) calculateNumberOfFieldsScoped(obj interface{}) int {
	return len(db.NewScope(obj).Fields())
}


func (db *OwnDb) batchScopedInsert(objArr []models.ScopedAggregatedProjection) (int64, error) {
	// If there is no data, nothing to do.
	if len(objArr) == 0 {
		return 0, errors.New("slice must not be empty")
	}
