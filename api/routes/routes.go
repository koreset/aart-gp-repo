...
func ConfigureRouter(router *gin.Engine) {
...

	{
		...
		groupPricing := apiv1.Group("group-pricing", NoCache())
		{
			...
			groupPricing.POST("quotes/:id/not-taken-up", controllers.MarkGroupPricingQuoteNotTakenUp)
			groupPricing.POST("quotes/:id/decline-quote", controllers.DeclineGroupPricingQuote)
			...
			groupPricing.GET("occupation-classes", controllers.GetGroupPricingOccupationClasses)
			...
			// Fraud risk scoring (GLM + company-configurable rules)
			groupPricing.POST("claims/:claim_id/fraud-check", controllers.RunFraudCheck)
			groupPricing.GET("fraud-risk/model", controllers.GetFraudRiskModel)
			groupPricing.POST("fraud-risk/refit", controllers.RefitFraudRiskModel)
			groupPricing.GET("fraud-risk/features", controllers.GetFraudFeatureCatalogue)
			groupPricing.GET("fraud-risk/rules", controllers.ListFraudRiskRules)
			groupPricing.POST("fraud-risk/rules", controllers.CreateFraudRiskRule)
			groupPricing.PUT("fraud-risk/rules/:rule_id", controllers.UpdateFraudRiskRule)
			groupPricing.DELETE("fraud-risk/rules/:rule_id", controllers.DeleteFraudRiskRule)
			...
			// Payment schedule lifecycle (Phase 1 — tighten claims-finance handover)
			groupPricing.POST("claims/payment-schedules/:schedule_id/signoff", RequirePermission("claims_pay:signoff_schedule"), controllers.SignOffPaymentSchedule)
			groupPricing.POST("claims/payment-schedules/:schedule_id/finance/start-review", RequirePermission("claims_pay:finance_review"), controllers.StartFinanceReview)
			groupPricing.POST("claims/payment-schedules/:schedule_id/items/:item_id/verify", RequirePermission("claims_pay:finance_review"), controllers.VerifyScheduleLineItem)
			groupPricing.POST("claims/payment-schedules/:schedule_id/items/:item_id/query", RequirePermission("claims_pay:finance_review"), controllers.QueryScheduleLineItem)
			groupPricing.POST("claims/payment-schedules/:schedule_id/items/:item_id/reject", RequirePermission("claims_pay:finance_review"), controllers.RejectScheduleLineItem)
			groupPricing.POST("claims/payment-schedules/:schedule_id/finance/authorise-first", RequirePermission("claims_pay:authorise_first"), controllers.FirstAuthorisePaymentSchedule)
			...
			groupPricing.POST("claims/authority-matrix", RequirePermission("claims_pay:admin_authority"), controllers.CreateAuthorityMatrixRow)
			groupPricing.PATCH("claims/authority-matrix/:row_id", RequirePermission("claims_pay:admin_authority"), controllers.UpdateAuthorityMatrixRow)
			groupPricing.DELETE("claims/authority-matrix/:row_id", RequirePermission("claims_pay:admin_authority"), controllers.DeleteAuthorityMatrixRow)
			// Payment cut-off settings + manual run (Phase 2)
			groupPricing.GET("claims/payment-cutoff/config", controllers.GetPaymentCutoffConfig)
			groupPricing.PUT("claims/payment-cutoff/config", RequirePermission("claims_pay:admin_cutoff"), controllers.SavePaymentCutoffConfig)
			groupPricing.POST("claims/payment-cutoff/run", RequirePermission("claims_pay:run_cutoff"), controllers.RunPaymentCutoffNow)
			groupPricing.GET("claims/payment-cutoff/runs", controllers.ListPaymentCutoffRuns)
			groupPricing.GET("claims/payment-cutoff/next", controllers.GetNextPaymentCutoff)
			// Sanctions / PEP screening (Phase 3)
			groupPricing.GET("claims/payment-schedules/:schedule_id/sanctions", controllers.ListScheduleSanctionsScreenings)
			groupPricing.POST("claims/payment-schedules/:schedule_id/items/:item_id/screen", RequirePermission("claims_pay:screen_sanctions"), controllers.ScreenScheduleLineItem)
			groupPricing.POST("claims/payment-schedules/:schedule_id/items/:item_id/sanctions-outcome", RequirePermission("claims_pay:screen_sanctions"), controllers.RecordSanctionsOutcome)
			// Reinsurance recovery (Phase 3)
			groupPricing.PUT("claims/payment-schedules/:schedule_id/items/:item_id/reinsurance", RequirePermission("claims_pay:finance_review"), controllers.SetReinsuranceRecovery)
			groupPricing.POST("claims/payment-schedules/:schedule_id/items/:item_id/reinsurance/raised", RequirePermission("claims_pay:recovery_confirm"), controllers.ConfirmReinsuranceRecoveryRaised)
			// Duplicate beneficiary clear (Phase 3)
			groupPricing.POST("claims/payment-schedules/:schedule_id/items/:item_id/duplicate/clear", RequirePermission("claims_pay:finance_review"), controllers.ClearDuplicateBeneficiary)
			...
			groupPricing.POST("claims/:claim_id/finance-rejection/acknowledge", RequirePermission("claims:assess"), controllers.AcknowledgeFinanceRejection)
			// Pre-authorisation verification suite (Phase 5)
			groupPricing.GET("claims/payment-schedules/:schedule_id/items/:item_id/bank-verification", RequirePermission("claims_pay:finance_review"), controllers.GetLineBankVerificationStatus)
			groupPricing.POST("claims/payment-schedules/:schedule_id/items/:item_id/bank-verification/reverify", RequirePermission("claims_pay:finance_review"), controllers.ReverifyLineBankAccount)
			groupPricing.POST("claims/payment-schedules/:schedule_id/items/:item_id/amount-drift/acknowledge", RequirePermission("claims_pay:finance_review"), controllers.AcknowledgeAmountDrift)
			// Claim payment confirmation letters (Phase 5)
			groupPricing.GET("claims/:claim_id/payment-letter.docx", RequirePermission("claims_pay:generate_letter"), controllers.DownloadClaimPaymentLetterDocx)
			groupPricing.GET("claims/:claim_id/payment-letter.pdf", RequirePermission("claims_pay:generate_letter"), controllers.DownloadClaimPaymentLetterPdf)
			groupPricing.GET("claims/:claim_id/payment-letter/history", RequirePermission("claims_pay:generate_letter"), controllers.ListClaimPaymentLetters)
			groupPricing.POST("claims/:claim_id/payment-letter/send", RequirePermission("claims_pay:send_letter"), controllers.SendClaimPaymentLetter)
			groupPricing.GET("claims/payment-schedules/:schedule_id/payment-letters.zip", RequirePermission("claims_pay:generate_letter"), controllers.DownloadScheduleLetterBundle)
			groupPricing.GET("settings/payment-letter", RequirePermission("claims_pay:configure_letter_settings"), controllers.GetPaymentLetterSettings)
			groupPricing.PUT("settings/payment-letter", RequirePermission("claims_pay:configure_letter_settings"), controllers.UpdatePaymentLetterSettings)
			...
			treaties := groupPricing.Group("reinsurance/treaties")
			{
				...
				treaties.GET("/scheme-link-conflicts", controllers.ListSchemeTreatyConflicts)
