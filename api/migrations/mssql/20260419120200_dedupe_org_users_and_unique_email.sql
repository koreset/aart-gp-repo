...
-- upsert keyed by email. Without this index the upsert silently
...
-- 3. Add the unique index if it doesn't already exist.
IF NOT EXISTS (
    SELECT 1 FROM sys.indexes
    WHERE name = 'idx_org_users_email'
      AND object_id = OBJECT_ID('org_users')
)
BEGIN
    CREATE UNIQUE INDEX idx_org_users_email ON org_users (email);
END;
