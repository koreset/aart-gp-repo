...
-- Step 1: mark the claim + populate the snapshot.
UPDATE group_scheme_claims gsc
INNER JOIN (
...
INNER JOIN claim_payment_schedules s
    ON s.id = q.schedule_id
SET gsc.status                            = 'finance_rejected',
    gsc.finance_rejected_at               = q.raised_at,
    gsc.finance_rejected_by               = q.raised_by,
    gsc.finance_rejection_reason_code     = q.reason_code,
    gsc.finance_rejection_notes           = q.notes,
    gsc.finance_rejection_schedule_number = s.schedule_number
...
-- Step 2: write a status_audit row for each backfilled claim so the
-- timeline reads honestly. Guarded by NOT EXISTS so re-runs don't
...
SELECT gsc.id,
       'approved',
       'finance_rejected',
       CONCAT('Backfill: historical finance rejection on ', gsc.finance_rejection_schedule_number, ' — ', gsc.finance_rejection_reason_code),
       'system_backfill',
       NOW()
