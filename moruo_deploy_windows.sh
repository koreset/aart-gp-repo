#!/usr/bin/env bash
# Windows (Git Bash) version of moruo_deploy.sh.
# Uses OpenSSH's scp (bundled with Windows 10/11) instead of rsync.
#
# Note: scp uses -P for port (capital), ssh uses -p (lowercase). Don't mix them up.
# Note: rsync's --delete is dropped because each release writes into a fresh
#       /opt/aart-gr/releases/$RELEASE directory — there is nothing to prune.
...
#    scp -r copies the directory itself into the destination, mirroring rsync's
#    "trailing-slash" source behavior when paired with a freshly-created release dir.
scp -r -P 2222 \
  api/migrations \
  moruogrsadmin@102.133.230.104:/opt/aart-gr/releases/$RELEASE/
