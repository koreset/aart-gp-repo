...
// Single dialog used for both single-version and bulk deletes. The confirm
// callback closes over the chosen action so the markup stays simple.
...
async function deleteVersion(t: any) {
  ...
  try {
    await GroupPricingService.deleteInsurerQuoteTemplate(insurerId.value, t.id)
    showSuccess(`Deleted version ${t.version}`)
    await loadAll()
  } catch (e: any) {
    console.error(e)
    // Backend returns 409 with a clear message when the active template is
    // targeted. Surface that message directly so the user understands why.
    const msg =
...
function confirmDeleteActive() {
  ...
  deleteDialog.message = `"${t.filename}" is the active template. Deleting it will revert quote generation to the system's default built-in layout. This cannot be undone.`
  deleteDialog.confirm = () => {
    deleteDialog.open = false
    deleteActive()
  }
...
async function deleteActive() {
  ...
  try {
    await GroupPricingService.deleteInsurerQuoteTemplate(
      insurerId.value,
      active.value.id,
      { force: true }
    )
...
async function deleteAllInactive() {
  ...
  try {
    const r = await GroupPricingService.deleteInactiveInsurerQuoteTemplates(
      insurerId.value
    )
