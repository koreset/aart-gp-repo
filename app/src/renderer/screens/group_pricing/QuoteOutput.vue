      ...
      <v-col>
        <base-card :show-actions="false">
          <template #header>
            <div class="d-flex align-center w-100">
              ...
              <span class="headline ml-4"
                >Generate Quotation for {{ quote.scheme_name }}</span
              >
              <v-spacer></v-spacer>
              <v-menu>
                ...
                <v-list density="compact">
                  ...
                  ></v-list-item>
                  <v-list-item
                    :prepend-icon="showGridView ? 'mdi-table-off' : 'mdi-table'"
                    :title="showGridView ? 'Hide Grid View' : 'Show Grid View'"
          ...
          <template #default>
            ...
            <!-- Template chooser -->
            <v-row v-if="!selectedTemplate" class="mx-7 mb-4">
              <v-col cols="12" class="pb-0">
                <h3 class="text-h6 font-weight-bold mb-1">
                  Select a quotation template
                </h3>
                <div class="text-body-2 text-medium-emphasis mb-2">
                  Choose how this quotation should be presented before
                  downloading.
                </div>
              </v-col>
              <v-col cols="12" md="6">
                <v-card
                  ...
                  rounded="lg"
                  hover
                  class="template-card pa-2 h-100"
                  @click="selectTemplate('system')"
                >
                  <v-card-item>
                    <template #prepend>
                    ...
                    <v-card-title class="font-weight-bold"
                      >System Template</v-card-title
                    >
                    <v-card-subtitle>AART standard layout</v-card-subtitle>
                  </v-card-item>
                  <v-card-text class="text-body-2">
                    Edit the introduction and underwriting provisions before
                    downloading. Output is generated by the application.
                  </v-card-text>
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn
                      ...
                      @click.stop="selectTemplate('system')"
                      >Use System Template</v-btn
                    >
                  </v-card-actions>
                </v-card>
              </v-col>
              <v-col cols="12" md="6">
                ...
                >
                  <v-card-item>
                    ...
                    <v-card-title class="font-weight-bold"
                      >Company Template</v-card-title
                    >
                    <v-card-subtitle>Insurer's branded layout</v-card-subtitle>
                  </v-card-item>
                  <v-card-text class="text-body-2">
                    Use the active .docx template uploaded for the insurer under
                    Insurer Settings. Layout and styling come from that
                    template.
                  </v-card-text>
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn
                      ...
                      variant="tonal"
                      append-icon="mdi-arrow-right"
                      @click.stop="selectTemplate('company')"
                      >Use Company Template</v-btn
                    >
                  </v-card-actions>
                </v-card>
              </v-col>
            ...
            <v-row v-if="selectedTemplate === 'system'" class="mx-7 mb-4">
              <v-col cols="12">
                <v-card variant="outlined" rounded="lg">
                  <v-card-item>
                    <template #prepend>
                    ...
                    >
                    <v-card-subtitle
                      >AART standard layout — editable</v-card-subtitle
                    >
                    <template #append>
                      <v-btn
                        size="small"
            ...
            <!-- Company Template view -->
            <v-row v-if="selectedTemplate === 'company'" class="mx-7 mb-4">
              <v-col cols="12">
                <v-card variant="outlined" rounded="lg">
                  <v-card-item>
                    <template #prepend>
                    ...
                    >
                    <v-card-subtitle>
                      Using the active template
                      <span v-if="insurer?.name">for {{ insurer.name }}</span>
                    </v-card-subtitle>
                    <template #append>
                      <v-btn
                        size="small"
                  ...
                  <v-card-text class="text-body-2 text-medium-emphasis">
                    The downloaded document uses the insurer's uploaded
                    <code>.docx</code> template. To change the styling, upload a
                    new version under Insurer Settings → Quote Template.
                    <span class="d-block mt-1">
                      Word documents can't be rendered inline in the browser —
                      use Preview to see the rendered layout (the same layout
                      both downloads use).
                    </span>
                  </v-card-text>
                  <v-card-actions class="px-4 pb-4">
                    ...
                    <v-btn
                      variant="text"
                      prepend-icon="mdi-eye-outline"
                      :loading="isPreviewingCompany"
                      :disabled="!quote"
                      @click="previewCompanyPdf"
                      >Preview</v-btn
                    >
                    <v-btn
                      class="ml-2"
                      ...
                      variant="flat"
                      prepend-icon="mdi-file-pdf-box"
                      :loading="isGeneratingBackendPdf"
                      :disabled="!quote"
            ...
            <v-row v-if="backendErrorMessage" class="mx-7">
              <v-col>
                <v-alert
                  type="error"
                  variant="tonal"
                  closable
                  @click:close="backendErrorMessage = ''"
                >
                  {{ backendErrorMessage }}
                </v-alert>
              </v-col>
            ...
            <div
              v-if="isPreviewingCompany && selectedTemplate === 'company'"
              ref="previewLoadingRef"
              class="mx-7 pdf-viewer pdf-viewer--loading d-flex flex-column align-center justify-center"
            >
              <v-progress-circular
                indeterminate
                color="primary"
              ...
              <div class="text-body-2 text-medium-emphasis mt-4">
                Rendering preview from the company template…
              </div>
              <div class="text-caption text-medium-emphasis mt-1">
                This can take 5–15 seconds the first time (Word warms up).
              </div>
            </div>
            <div
              v-else-if="pdfUrl && selectedTemplate"
              ref="previewIframeRef"
              class="mx-7 pdf-viewer"
            >
              <iframe
                :src="pdfUrl"
                width="100%"
...
const generateWordFromBackend = async () => {
  ...
  try {
    ...
    backendErrorMessage.value = await extractBlobErrorMessage(
      error,
...
const buildSystemPdf = async (): Promise<any> => {
...
const systemPdfFilename = () =>
  `${quote.value?.scheme_name || 'Quotation'}_Quotation.pdf`

const viewSystemPdf = async () => {
  const doc = await buildSystemPdf()
  const pdfBlob = doc.output('blob')
  if (pdfUrl.value) URL.revokeObjectURL(pdfUrl.value)
  pdfUrl.value = URL.createObjectURL(pdfBlob)
}

const downloadSystemPdf = async () => {
  const doc = await buildSystemPdf()
  doc.save(systemPdfFilename())
}

const isGeneratingBackendPdf = ref(false)
const backendErrorMessage = ref('')

// Backend endpoints requested with responseType: 'blob' return error bodies
// as a Blob too. Read it as text and pull out the JSON error message so the
// user sees something useful instead of a silent failure.
const extractBlobErrorMessage = async (
  error: any,
  fallback: string
): Promise<string> => {
...
const downloadCompanyPdf = async () => {
  if (!quote.value?.id) return
  isGeneratingBackendPdf.value = true
  backendErrorMessage.value = ''
  try {
    const response = await GroupPricingService.getQuotePdf(quote.value.id)
    let filename = `${quote.value.scheme_name || 'Quotation'}_Quotation.pdf`
    const cd = response.headers?.['content-disposition']
    if (cd) {
      const match = /filename="?([^";]+)"?/i.exec(cd)
      if (match?.[1]) filename = match[1]
    ...
    console.error('Error generating backend PDF document:', error)
    backendErrorMessage.value = await extractBlobErrorMessage(
      error,
      'Failed to generate PDF.'
    )
  } finally {
    isGeneratingBackendPdf.value = false
  }
}

const isPreviewingCompany = ref(false)
const previewLoadingRef = ref<HTMLElement | null>(null)
const previewIframeRef = ref<HTMLElement | null>(null)
// Cached PDF blob keyed by quote id so re-clicks within a session are instant.
let companyPreviewCache: { quoteId: number; blob: Blob } | null = null

const scrollPreviewIntoView = async () => {
  await nextTick()
  const target = previewIframeRef.value || previewLoadingRef.value
  target?.scrollIntoView({ behavior: 'smooth', block: 'start' })
}

const previewCompanyPdf = async () => {
  if (!quote.value?.id) return
  backendErrorMessage.value = ''

  if (companyPreviewCache && companyPreviewCache.quoteId === quote.value.id) {
    if (pdfUrl.value) URL.revokeObjectURL(pdfUrl.value)
    pdfUrl.value = URL.createObjectURL(companyPreviewCache.blob)
    await scrollPreviewIntoView()
    return
  }

  isPreviewingCompany.value = true
  await scrollPreviewIntoView()
  try {
    const response = await GroupPricingService.getQuotePdf(quote.value.id)
    const blob =
    ...
    companyPreviewCache = { quoteId: quote.value.id, blob }
    if (pdfUrl.value) URL.revokeObjectURL(pdfUrl.value)
    pdfUrl.value = URL.createObjectURL(blob)
  } catch (error) {
    console.error('Error previewing backend PDF document:', error)
    backendErrorMessage.value = await extractBlobErrorMessage(
      error,
      'Failed to render preview.'
    )
  } finally {
    isPreviewingCompany.value = false
    await scrollPreviewIntoView()
  }
}

const selectedTemplate = ref<'system' | 'company' | null>(null)
const selectTemplate = (template: 'system' | 'company') => {
