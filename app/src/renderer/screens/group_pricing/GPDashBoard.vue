        ...
        <base-card :show-actions="false">
          ...
          <template #default>
            ...
            <v-window v-model="dashboardTab">
              <v-window-item value="quoting" eager>
                <v-row class="align-center">
                  <v-col cols="4">
                    <v-select
                      v-model="selectedYearQuoting"
                      variant="outlined"
                      density="compact"
                      :items="availableYears"
                      label="Select Year"
                      placeholder="Select Year"
                      @update:model-value="refreshQuoting"
                    ></v-select>
                  </v-col>
                  <v-col v-if="financialYearInfoQuoting" cols="auto">
                    <v-chip
                      size="small"
                      color="primary"
                      variant="tonal"
                    ...
                    >
                      {{ financialYearInfoQuoting.financial_year_label }}
                    </v-chip>
                  </v-col>
                </v-row>
                ...
                <!-- ── Quoting KPI: Conversion Rate ──────────────────────────── -->
                <v-row class="d-flex justify-center">
                  <v-col
                    v-for="card in cardsQuoting"
                    :key="card.title"
                    :cols="card.flex"
                  >
                    <v-card
                ...
                >
                  <strong>Quotes Data</strong> — Sections below show activity
                  across all quotes (all statuses) for the selected year.
                </v-alert>
                  ...
                  <v-col cols="6" class="card-bg mx-1">
                    ...
                    >
                      ...
                      <v-tooltip location="bottom" max-width="300">
                        ...
                        <span>
                          ...
                          Percentage of quotes correctly classified as won or
                          lost on the held-out test set.<br /><br />
                      ...
                      <v-card>
                        ...
                        <v-card-text>
                          <p class="mb-3">
                            ...
                            <strong
                              >separates won quotes from lost quotes</strong
                            >
                            across all possible score thresholds — independent
                            of any fixed cut-off.
                          </p>
                          ...
                          <v-table density="compact" class="mb-3">
                            ...
                            <tbody>
                              <tr
                                ><td>0.50</td
                                ><td>No better than random guessing</td
                                ><td class="text-error">None</td></tr
                              >
                              <tr
                                ><td>0.50–0.65</td
                                ><td>Weak signal, limited data</td
                          ...
                          <p class="text-caption text-grey-darken-1">
                            AUC is more reliable than accuracy when wins and
                            losses are imbalanced (e.g. 80% of quotes are lost).
                            A high AUC means the model correctly ranks a
                            randomly chosen won quote above a randomly chosen
                            lost quote.
                          </p>
                        </v-card-text>
                        <v-divider></v-divider>
                    ...
                    >
                      <v-card>
                        ...
                        <v-card-text>
                          <p class="mb-3">
                            Training size is the number of historical quotes
                            (with known outcomes) used to fit the model. More
                            data means the model's weights are
                          ...
                          <v-table density="compact" class="mb-3">
                            ...
                            <tbody>
                              ...
                              <tr
                                ><td>50–99</td
                                ><td class="text-info">Moderate</td
                              ...
                              <tr
                                ...
                                ><td
                                  >Reliable for underwriting and sales
                                  decisions</td
                                ></tr
                              >
                              <tr
                          ...
                          <p class="text-caption text-grey-darken-1">
                            The model uses an 80/20 chronological train/test
                            split, so the displayed size reflects the training
                            portion only. As more quotes reach terminal status
                            (accepted, rejected, expired etc.), retrain the
                            model to improve credibility.
                          </p>
                        </v-card-text>
                        <v-divider></v-divider>
                        <v-card-actions>
                  ...
                  <v-card>
                    ...
                    <v-card-text class="pt-4">
                      ...
                      >
                        ...
                        in-force, rejected, not-taken-up, or expired) are
                        required. Training will be skipped silently if this
                        threshold is not met.
                      </v-alert>
                      <p class="mt-3 text-caption text-grey-darken-1">
                        Training runs in the background and may take a few
                        seconds. All active quotes will be automatically
                        rescored once training completes.
                      </p>
                    </v-card-text>
                    <v-divider></v-divider>
                    <v-card-actions>
                ...
                <!-- ── Quoting tab — Exposure Analysis ─────────────────────── -->
                <v-row class="align-center mb-2">
                  <v-col>
                    <span class="text-subtitle-1 font-weight-bold"
                      >Exposure Analysis</span
                    >
                  </v-col>
                  <v-col cols="auto" class="d-flex align-center gap-2">
                    <v-tooltip
                      :text="exposureLockTooltipQuoting"
                      location="bottom"
                    >
                      <template #activator="{ props: ttp }">
                        <span v-bind="ttp">
                          <v-btn
                            variant="tonal"
                            :color="
                              isSelectedFYOpenQuoting ? 'primary' : 'grey'
                            "
                            size="small"
                            :prepend-icon="
                              isSelectedFYOpenQuoting
                                ? 'mdi-refresh'
                                : 'mdi-lock-outline'
                            "
                            :loading="rebuildLoadingQuoting"
                            :disabled="!isSelectedFYOpenQuoting"
                            @click="triggerExposureRebuildQuoting"
                          >
                            {{
                              isSelectedFYOpenQuoting
                                ? 'Refresh Exposure'
                    ...
                    <v-chip
                      v-if="
                        !isSelectedFYOpenQuoting && financialYearInfoQuoting
                      "
                      size="x-small"
                      color="grey"
                      variant="tonal"
                    ...
                    >
                      {{ financialYearInfoQuoting.financial_year_label }} closed
                    </v-chip>
                  </v-col>
                </v-row>
                ...
                <v-row class="mt-4 align-center">
                  <v-col cols="3">
                    <v-select
                      v-model="selectedBenefitQuoting"
                      :items="benefitList"
                      variant="outlined"
                      density="compact"
                      label="Benefit"
                  ...
                  <v-col cols="4">
                    <v-btn-toggle
                      v-model="exposureDataSourceQuoting"
                      mandatory
                      density="compact"
                      variant="outlined"
                      color="primary"
                    ...
                    >
                      <v-tooltip
                        text="Every quote this year — pipeline, lost, accepted and in-force (no filter on quote status)"
                        location="bottom"
                      >
                        <template #activator="{ props: ttp }">
                          <v-btn v-bind="ttp" value="all" size="small"
                            >All Quotes</v-btn
                          >
                        </template>
                      </v-tooltip>
                      <v-tooltip
                        text="Quotes excluding the in-force book — open pipeline, rejected, expired, not-taken-up"
                        location="bottom"
                      >
                        <template #activator="{ props: ttp }">
                          <v-btn v-bind="ttp" value="quotes" size="small"
                            >Excl. In-Force</v-btn
                          >
                        </template>
                      </v-tooltip>
                  ...
                  <v-col cols="auto">
                    <v-btn
                      ...
                      :loading="refreshLoadingQuoting"
                      @click="refreshQuoting"
                    >
                      Refresh
                    </v-btn>
                  </v-col>
                ...
                <v-row>
                  <v-col cols="5.1" class="card-bg mx-1">
                    <ChartMenu
                      :chart-ref="expChartsQuoting"
                      :title="`${selectedBenefitQuoting} Exposure by Age Band — ${exposureDataSourceLabelQuoting}`"
                      :data="exposureOptionsQuoting?.data ?? []"
                    />
                    <ag-charts
                      v-if="exposureOptionsQuoting"
                      ref="expChartsQuoting"
                      :options="exposureOptionsQuoting"
                    ></ag-charts>
                  </v-col>
                  <v-col cols="5.1" class="card-bg mx-1">
                    <ChartMenu
                      :chart-ref="expGenderChartsQuoting"
                      :title="`${selectedBenefitQuoting} Exposure by Age Band & Gender — ${exposureDataSourceLabelQuoting}`"
                      :data="exposureGenderOptionsQuoting?.data ?? []"
                    />
                    <ag-charts
                      v-if="exposureGenderOptionsQuoting"
                      ref="expGenderChartsQuoting"
                      :options="exposureGenderOptionsQuoting"
                    ></ag-charts>
                  </v-col>
                </v-row>
                ...
                <v-row class="mt-4">
                  <v-col cols="11.2" class="card-bg mx-1">
                    ...
                    >
                      <ChartMenu
                        :chart-ref="exposureTrendChartsQuoting"
                        :title="`${selectedBenefitQuoting} Exposure Trend by Year & Age Band — ${exposureDataSourceLabelQuoting}`"
                        :data="exposureTrendOptionsQuoting?.data ?? []"
                      />
                    </div>
                    <ag-charts
                      v-if="exposureTrendOptionsQuoting"
                      ref="exposureTrendChartsQuoting"
                      :options="exposureTrendOptionsQuoting"
                    ></ag-charts>
                ...
                <v-row class="mt-4">
                  <v-col cols="5.1" class="card-bg mx-1">
                    ...
                    >
                      <ChartMenu
                        :chart-ref="provinceChartsQuoting"
                        :title="`${selectedBenefitQuoting || 'All'} Exposure by Region — ${exposureDataSourceLabelQuoting}`"
                        :data="provinceBarOptionsQuoting?.data ?? []"
                      />
                      <v-select
                        v-model="provinceViewByQuoting"
                        :items="viewByOptions"
                        density="compact"
                        variant="outlined"
                        hide-details
              ...
              <v-window-item value="inforce" eager>
                <v-row class="align-center">
                  <v-col cols="4">
                    <v-select
                      v-model="selectedYearInforce"
                      variant="outlined"
                      density="compact"
                      :items="availableYears"
                      label="Select Year"
                      placeholder="Select Year"
                      @update:model-value="refreshInforce"
                    ></v-select>
                  </v-col>
                  <v-col v-if="financialYearInfoInforce" cols="auto">
                    <v-chip
                      size="small"
                      color="primary"
                      variant="tonal"
                      prepend-icon="mdi-calendar-range"
                    >
                      {{ financialYearInfoInforce.financial_year_label }}
                    </v-chip>
                  </v-col>
                </v-row>
                ...
                >
                  <strong>In-Force Portfolio</strong> — Sections below reflect
                  the accepted and in-force book. Use the Data Source toggle on
                  exposure charts to switch between In-Force, Quotes-only, or
                  All data.
                </v-alert>
                ...
                <v-row class="align-center mb-2">
                  ...
                  <v-col cols="auto" class="d-flex align-center gap-2">
                    ...
                    <v-chip
                      v-if="!isSelectedFYOpen && financialYearInfoInforce"
                      size="x-small"
                      color="grey"
                      variant="tonal"
                      prepend-icon="mdi-lock-outline"
                    >
                      {{ financialYearInfoInforce.financial_year_label }} closed
                    </v-chip>
                  </v-col>
                </v-row>
                <v-row class="d-flex justify-center">
                ...
                <v-row class="mt-4 align-center">
                  ...
                  <v-col cols="auto">
                    <v-btn
                      ...
                      size="small"
                      prepend-icon="mdi-refresh"
                      :loading="refreshLoadingInforce"
                      @click="refreshInforce"
                    >
                      Refresh
                    </v-btn>
                  </v-col>
                  ...
                  <v-col cols="5.1" class="card-bg mx-1">
                    ...
                    >
                      <ChartMenu
                        ...
                        :title="`${selectedBenefit || 'All'} Exposure by Region — ${exposureDataSourceLabel}`"
...
const selectedYearQuoting = ref<string | null>(null)
const selectedYearInforce = ref<string | null>(null)
...
const selectedBenefitQuoting = ref<string | null>(null)
// Inforce tab data source is fixed to 'inforce' (the toggle has been moved
// to the Quoting tab where it cycles between 'all' and 'quotes').
const exposureDataSource = ref<string>('inforce')
const exposureDataSourceQuoting = ref<string>('all')

// Section banner visibility
const showQuotesBanner = ref(true)
const showInforceBanner = ref(true)

// Financial year info from backend (per tab — each year selector is independent)
const financialYearInfoQuoting = ref<any>(null)
const financialYearInfoInforce = ref<any>(null)

// Exposure rebuild state (one per tab — both call the same backend rebuild,
// then the relevant tab's exposure data is refetched).
const rebuildLoading = ref(false)
const rebuildLoadingQuoting = ref(false)

// Per-tab refresh loading — drives the Refresh button spinner so the user
// gets visual feedback that data is being re-fetched after benefit changes.
const refreshLoadingInforce = ref(false)
const refreshLoadingQuoting = ref(false)
...
const expChartsQuoting: any = ref(null)
const expGenderChartsQuoting: any = ref(null)
const exposureTrendChartsQuoting: any = ref(null)
const provinceChartsQuoting: any = ref(null)
const industryAgeChartsQuoting: any = ref(null)
...
const exposureTrendOptionsQuoting: any = ref<AgChartOptions | null>(null)
...
let quotingData: any = null
let inforceData: any = null
...
onMounted(() => {
  ...
  selectedYearQuoting.value = currentYear
  selectedYearInforce.value = currentYear
  selectedBenefit.value = 'All'
  selectedBenefitQuoting.value = 'All'
...
// Status bar reflects whichever tab is active
watchEffect(() => {
  const year =
    dashboardTab.value === 'quoting'
      ? selectedYearQuoting.value
      : selectedYearInforce.value
  const items: any[] = [{ icon: 'mdi-calendar', text: `Year: ${year ?? '—'}` }]
  if (dashboardTab.value === 'inforce' && selectedBenefit.value) {
    items.push({
      icon: 'mdi-shield-check-outline',
...
// ── Quoting tab — exposure chart options (same shape as Inforce versions) ────

const exposureOptionsQuoting: any = ref<AgChartOptions>({
  theme: paperTheme,
  data: [],
  title: {
    text: `${selectedBenefitQuoting.value} Exposure`,
    fontSize: 14,
    fontWeight: 'bold'
  },
  legend: { enabled: true },
...
const exposureGenderOptionsQuoting: any = ref<AgChartOptions>({
  data: [],
  title: {
    text: `${selectedBenefitQuoting.value} Exposure`,
    fontSize: 14,
...
const rawProvinceDataQuoting = ref<any[]>([])
const rawIndustryAgeDataQuoting = ref<any[]>([])
...
const provinceViewByQuoting = ref<'Count' | 'Sum Assured'>('Count')
const industryAgeViewByQuoting = ref<'Count' | 'Sum Assured'>('Sum Assured')
...
const exposureDataSourceLabelQuoting = computed(() =>
  exposureDataSourceQuoting.value === 'quotes' ? 'Excl. In-Force' : 'All Quotes'
)
...
const isSelectedFYOpen = computed(() => {
  const info = financialYearInfoInforce.value
...
const exposureLockTooltip = computed(() =>
  isSelectedFYOpen.value
    ...
    : `${financialYearInfoInforce.value?.financial_year_label ?? 'This financial year'} is closed — historical data is locked`
)
...
// ── Quoting tab — FY-open guard for the Quoting Refresh Exposure button ──────
const isSelectedFYOpenQuoting = computed(() => {
  const info = financialYearInfoQuoting.value
  if (!info) return false

  const yem: number = info.year_end_month
  const fyStartYear: number = info.financial_year_start
  const fyEndYear: number = info.financial_year_end

  const startMonth = (yem % 12) + 1
  const fyStart = new Date(fyStartYear, startMonth - 1, 1)
  const fyEnd = new Date(fyEndYear, yem, 0)
  fyEnd.setHours(23, 59, 59, 999)
...
const exposureLockTooltipQuoting = computed(() =>
  isSelectedFYOpenQuoting.value
    ? 'Rebuild exposure data for this financial year'
    : `${financialYearInfoQuoting.value?.financial_year_label ?? 'This financial year'} is closed — historical data is locked`
)

// ── Quoting tab — region & industry/age computed options ─────────────────────
const provinceBarOptionsQuoting = computed<any>(() => {
  if (!rawProvinceDataQuoting.value.length) return undefined
...
const industryAgeOptionsQuoting = computed<any>(() => {
  if (!rawIndustryAgeDataQuoting.value.length) return undefined
  const isCount = industryAgeViewByQuoting.value === 'Count'
  const raw = rawIndustryAgeDataQuoting.value
  const valueKey = isCount ? 'record_count' : 'total_sum_assured'
...
const getExposureData = async () => {
  try {
    if (selectedBenefit.value && selectedYearInforce.value) {
      ...
      const titleText = `${selectedBenefit.value} Exposure (${selectedYearInforce.value}) — ${sourceLabel}`
...
const triggerExposureRebuild = async () => {
  if (!selectedYearInforce.value) return
  rebuildLoading.value = true
  try {
    await GroupPricingService.rebuildExposureData(selectedYearInforce.value)
...
// ── Quoting tab — exposure fetchers ───────────────────────────────────────────

const getExposureDataQuoting = async () => {
  try {
    if (selectedBenefitQuoting.value && selectedYearQuoting.value) {
      const res = await GroupPricingService.getExposureData(
        selectedYearQuoting.value,
        selectedBenefitQuoting.value,
        exposureDataSourceQuoting.value
      )
      const sourceLabel = exposureDataSourceLabelQuoting.value
      const titleText = `${selectedBenefitQuoting.value} Exposure (${selectedYearQuoting.value}) — ${sourceLabel}`
      exposureOptionsQuoting.value = {
        ...exposureOptionsQuoting.value,
        title: { text: titleText, fontSize: 14, fontWeight: 'bold' },
        data: res.data || []
      }
      exposureGenderOptionsQuoting.value = {
        ...exposureGenderOptionsQuoting.value,
        title: { text: titleText, fontSize: 14, fontWeight: 'bold' },
        data: res.data || []
      }
...
const getExposureTrendDataQuoting = async () => {
  try {
    const res = await GroupPricingService.getExposureTrend(
      selectedBenefitQuoting.value || 'All',
      exposureDataSourceQuoting.value
    ...
    const rows: any[] = res.data?.data || []
    if (!rows.length) {
      exposureTrendOptionsQuoting.value = null
      return
    }
    ...
    exposureTrendOptionsQuoting.value = {
      data: chartData,
      title: {
        text: `${selectedBenefitQuoting.value || 'All'} Exposure Trend by Year & Age Band — ${sourceLabel}`,
        fontSize: 14,
        fontWeight: 'bold'
      },
      series: ageBands.map((ab) => ({
...
const triggerExposureRebuildQuoting = async () => {
  if (!selectedYearQuoting.value) return
  rebuildLoadingQuoting.value = true
  try {
    await GroupPricingService.rebuildExposureData(selectedYearQuoting.value)
    await getExposureDataQuoting()
    await getExposureTrendDataQuoting()
  } catch (error) {
    console.error(error)
  } finally {
    rebuildLoadingQuoting.value = false
  }
}

// ── refreshQuoting ────────────────────────────────────────────────────────────
// Refreshes only the Quoting tab — pipeline charts, broker table,
// conversion donuts, win-probability distribution, and the Conversion Rate KPI.

const refreshQuoting = async () => {
  if (!selectedYearQuoting.value) return
  refreshLoadingQuoting.value = true
  try {
    await refreshQuotingInner()
  } finally {
    refreshLoadingQuoting.value = false
  }
}

const refreshQuotingInner = async () => {
  GroupPricingService.getFinancialYearInfo(selectedYearQuoting.value!)
    .then((res) => {
      if (res.data?.data) financialYearInfoQuoting.value = res.data.data
    })
    .catch(() => {})
  ...
  // New Quotes count — sum of all new-business statuses (matches the donut total)
  const newQuotesCount =
    (data.new_quotes_in_progress_count ?? 0) +
    (data.new_quotes_pending_review_count ?? 0) +
    (data.new_quotes_approved_count ?? 0) +
    (data.new_quotes_in_force_count ?? 0) +
    (data.new_quotes_accepted_count ?? 0)

  // Quoted Annual Premium — total premium across all quotes for the year
  const quotedPremium = Math.round(data.conversion_total_premium ?? 0)
  ...
  getExposureDataQuoting()
  getExposureTrendDataQuoting()
}

// ── refreshInforce ────────────────────────────────────────────────────────────
// Refreshes only the Inforce tab — KPI cards (Annual Premium, Scheme Count,
// Expected Claims, Expense Recovery, Expected Loss Ratio), portfolio analysis
// charts, exposure-by-region/industry, and pricing intelligence.

const refreshInforce = async () => {
  if (!selectedYearInforce.value) return
  refreshLoadingInforce.value = true
  try {
    await refreshInforceInner()
  } finally {
    refreshLoadingInforce.value = false
  }
}

const refreshInforceInner = async () => {
  GroupPricingService.getFinancialYearInfo(selectedYearInforce.value!)
    .then((res) => {
      if (res.data?.data) financialYearInfoInforce.value = res.data.data
    })
    .catch(() => {})
  ...
  inforceData = res.data
  const data = inforceData
  ...
  // KPI cards from backend — drop the two quoting-flavoured cards;
  // they live on the Quoting tab. Keep Schemes In Force, Gross Written
  // Premium, plus the computed Expected Loss Ratio.
  const HIDE_ON_INFORCE = new Set(['New Quotes', 'Quoted Annual Premium'])
  const backendCards = (res.data.card_data ?? [])
    .filter((c: any) => !HIDE_ON_INFORCE.has(c.title))
    .map((c: any) => ({ ...c, flex: 4 }))
